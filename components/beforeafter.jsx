...
const BeforeAfter = () => {
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(entries => {
      entries.forEach(en => {
        if (en.isIntersecting && !hasInteracted) {
          let frame=0, total=90;
          const tick = () => { frame++; const t=frame/total; setPos(t<0.5?80-(80-30)*(t/0.5):30+(50-30)*((t-0.5)/0.5)); if(frame<total&&!hasInteracted) requestAnimationFrame(tick); };
          setTimeout(()=>requestAnimationFrame(tick),400); io.disconnect();
        }
      });
    },{ threshold:0.4 });
  ...
  const onDown = e => { setHasInteracted(true); setDragging(true); updateFromClientX(e.touches?e.touches[0].clientX:e.clientX); };

  return (
    <section className="qd-section" data-bridge="obsidian-2"
      ...
      <div style={{ maxWidth:1280,margin:'0 auto',position:'relative' }}>
        <div style={{ display:'grid',gridTemplateColumns:'1.2fr 1fr',gap:60,alignItems:'flex-end',marginBottom:56 }}>
          <Reveal lift={32}>
        ...
        <Reveal lift={40} duration={1000}>
          <div ref={wrapRef} onMouseDown={onDown} onTouchStart={onDown}
            style={{ position:'relative',borderRadius:16,overflow:'hidden',border:'1px solid var(--border-1)',boxShadow:'0 24px 80px rgba(0,0,0,0.6)',cursor:dragging?'grabbing':'ew-resize',userSelect:'none',aspectRatio:'16/10',maxHeight:760,background:'var(--obsidian-2)' }}>
            <div style={{ position:'absolute',top:0,left:0,right:0,height:36,background:'var(--obsidian-3)',borderBottom:'1px solid var(--border-1)',display:'flex',alignItems:'center',padding:'0 14px',gap:8,zIndex:10 }}>
            ...
            <div style={{ position:'absolute',top:36,left:0,right:0,bottom:0 }}>
              <div style={{ position:'absolute',inset:0 }}><AfterSite /></div>
              <div style={{ position:'absolute',inset:0,clipPath:`polygon(0 0,${pos}% 0,${pos}% 100%,0 100%)`,transition:dragging?'none':'clip-path 80ms linear' }}><BeforeSite /></div>
              <div style={{ position:'absolute',top:-28,left:6,zIndex:5,fontFamily:'var(--font-mono)',fontSize:10,letterSpacing:'0.2em',color:'var(--fg3)',textTransform:'uppercase',opacity:pos>8?1:0,transition:'opacity 200ms' }}>● BEFORE · 2014</div>
              <div style={{ position:'absolute',top:-28,right:6,zIndex:5,fontFamily:'var(--font-mono)',fontSize:10,letterSpacing:'0.2em',color:'var(--acid)',textTransform:'uppercase',opacity:pos<92?1:0,transition:'opacity 200ms' }}>AFTER · QD ●</div>
              <div style={{ position:'absolute',top:0,bottom:0,left:`calc(${pos}% - 1px)`,width:2,background:'var(--acid)',boxShadow:'0 0 18px rgba(166,240,79,0.7),0 0 40px rgba(166,240,79,0.4)',pointerEvents:'none',zIndex:6 }} />
              <div style={{ position:'absolute',top:'50%',left:`${pos}%`,transform:'translate(-50%,-50%)',zIndex:7,width:56,height:56,borderRadius:'50%',background:'var(--acid)',color:'var(--obsidian)',display:'grid',placeItems:'center',boxShadow:'0 0 0 4px rgba(10,11,13,0.9),0 0 32px rgba(166,240,79,0.6)',cursor:dragging?'grabbing':'grab',animation:hasInteracted?'none':'qd-handle-bob 2.2s ease-in-out infinite' }}>
        ...
        <Reveal delay={120} lift={20}>
          <div style={{ marginTop:18,display:'flex',justifyContent:'center',gap:10 }}>
            {[10,25,50,75,90].map(p=>(
              <button key={p} onClick={()=>{ setHasInteracted(true); setPos(p); }}
                style={{ cursor:'pointer',padding:'6px 14px',background:Math.abs(pos-p)<3?'var(--acid)':'transparent',color:Math.abs(pos-p)<3?'var(--obsidian)':'var(--fg2)',border:`1px solid ${Math.abs(pos-p)<3?'var(--acid)':'var(--border-2)'}`,borderRadius:4,fontFamily:'var(--font-mono)',fontSize:10,fontWeight:600,letterSpacing:'0.12em',transition:'all 200ms ease' }}>{p}%</button>
            ))}
          </div>
        </Reveal>
